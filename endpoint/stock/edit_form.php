...
<form class="ui small modal form zn-form-update" action="stock/edit" data="<?=$_GET['id']?>">
	<div class="header">
		...
		<div class="ui field required">
			...
			<select class="ui dropdown multiple search" multiple name="supplier[]">
				<?php foreach (Supplier::getAll()->data as $item) { ?>
					<?php $found = false; ?>
					<?php foreach (_StockSupplier::getBy('Stock', $fetch['id'])->data as $_item) { ?>
						<?php if($_item['supplier'] == $item['id']) { ?>
							<?php $found = true; ?>
							<option <?=(true) ? 'selected' : ''?> value="<?=$item['id']?>"><?=$item['name']?></option>
						<?php } ?>
					<?php } ?>
					<?php if(!$found) { ?>
