			...
			<tbody>
				<?php foreach(Supplier::getAll() as $item) { ?>
					<tr>
						...
						<td><?= $item["address"] ?></td>
						<td>
							<a class="ui mini circular icon button violet zn-link-dialog" href="<?=Helper::url("api/supplier/view.php?id=".$item["id"])?>" data="<?=$item["id"]?>" data-tooltip="<?=Translator::translate("view details")?>">
								<i class="ui eye icon"></i>
							</a>
							<a class="ui mini circular icon button green zn-link-dialog" href="<?=Helper::url("api/supplier/edit_form.php?id=".$item["id"])?>" data="<?=$item["id"]?>" data-tooltip="<?=Translator::translate("edit details")?>">
								<i class="ui edit icon"></i>
							</a>
							<?php if(!_StockSupplier::getBy("supplier", $item["id"])): ?>
							<a class="ui mini circular icon button red zn-link-dialog" href="<?=Helper::url("api/supplier/delete_form.php?id=".$item["id"])?>" data="<?=$item["id"]?>" data-tooltip="<?=Translator::translate("delete")?>">
