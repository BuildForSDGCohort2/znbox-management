...
if(!$fetch = Supplier::getBy("id", $_GET["id"])) {
    die("404_request");   
}
?>
<form class="ui small modal form zn-form-update" action="<?=Helper::url("api/supplier/edit.php")?>" data="<?=$_GET["id"]?>">
	<div class="header">
		<h3 class="ui header diviving color red"><i class="ui dolly flabed icon"></i><?=Translator::translate("edit supplier details");?></h3>
	</div>
