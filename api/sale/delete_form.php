...
?>
<form class="ui mini modal form zn-form-update" action="<?=Helper::url("api/sale/edit.php")?>" data="<?=$_GET["id"]?>">
    <div class="header">
        <h3 class="ui header red"><?=Translator::translate("Do you wish to delete this item")?>?</h3>
    </div>
    <div class="content">
        <div class="ui header small">
            <h6><?=Helper::toRef($fetch["id"]) . " - " . Customer::getBy("id", $fetch["customer"])["name"]?></h6>
        </div>
